...
export function parseRems(workspace: Workspace): [TDoc, DocMap] {
  ...
  const childrenMap: Map<DocId, DocId[]> = new Map();
  ...
  for (const doc of docs) {
    ...
    const parent = childrenMap.get(doc.parent);
    if (!parent) {
      childrenMap.set(doc.parent, [id]);
    } else {
      parent.push(id);
    }
  }
  ...
  function buildDocTree(id: DocId) {
    const doc = docMap.get(id);
    if (!doc) {
      throw new Error(`No doc found for id ${id}`);
    }
    const children = childrenMap.get(id);
    if (!children) {
      return { val: doc, ch: [] };
    }
    const ch = children.map(buildDocTree);
    return { val: doc, ch };
  }

  const root = buildDocTree(workspace.documentRemToExportId);
...
export function transformDoc(
  ...
  if (tdoc.val._id === "WyIrrfcpeaSfm69tr")
    {
      console.log({
      tdoc,
      }) 
